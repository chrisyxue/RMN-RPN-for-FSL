...
def main():
    ...
    metatrain_folders,metatest_folders,metaval_folders = tg.cifar_folders()
    ...
    for episode in range(EPISODE):
        ...
        task = tg.CiFarTask(metatrain_folders,CLASS_NUM,SAMPLE_NUM_PER_CLASS,BATCH_NUM_PER_CLASS)
        sample_dataloader = tg.get_cifar_data_loader(task,num_per_class=SAMPLE_NUM_PER_CLASS,split="train",shuffle=False)
        batch_dataloader = tg.get_cifar_data_loader(task,num_per_class=BATCH_NUM_PER_CLASS,split="test",shuffle=True,train_query_argue=True)
        ...
        """------------------Phrase of RMN-----------------------------------------"""
        sample_features_ext = sample_features.unsqueeze(0).repeat(BATCH_NUM_PER_CLASS*CLASS_NUM,1,1,1,1)
        sample_features_ext = sample_features_ext.view(-1,FEATURE_DIM,6,6).contiguous() 
        batch_features_ext = batch_features.unsqueeze(0).repeat(CLASS_NUM,1,1,1,1) 
        batch_features_ext = torch.transpose(batch_features_ext,0,1) 
        batch_features_ext = batch_features_ext.contiguous().view(-1,FEATURE_DIM,6,6) 
        ...
        if episode%5000 == 0:
            ...
            for i in range(VAL_EPISODE):
                total_rewards = 0
                task = tg.CiFarTask(metaval_folders,CLASS_NUM,SAMPLE_NUM_PER_CLASS,15)
                sample_dataloader = tg.get_cifar_data_loader(task,num_per_class=SAMPLE_NUM_PER_CLASS,split="train",shuffle=False)
                num_per_class = 5
                val_dataloader = tg.get_cifar_data_loader(task,num_per_class=num_per_class,split="test",shuffle=False)
                ...
                for val_images,val_labels in val_dataloader:
                    ...
                    relation_pairs = torch.cat((sample_features_ext,val_features_ext),2).view(-1,FEATURE_DIM*2,6,6)
                    ...
                    """------------------RMN-----------------------------------------"""
                    sample_features_ext = sample_features.unsqueeze(0).repeat(batch_size,1,1,1,1)
                    sample_features_ext = sample_features_ext.view(-1,FEATURE_DIM,6,6) 

                    val_features_ext = val_features.unsqueeze(0).repeat(1*CLASS_NUM,1,1,1,1)
                    val_features_ext = torch.transpose(val_features_ext,0,1)
                    val_features_ext = val_features_ext.contiguous().view(-1,FEATURE_DIM,6,6)
        ...
        if episode%500 == 0:
            ...
            for i in range(TEST_EPISODE):
                total_rewards = 0
                task = tg.CiFarTask(metatest_folders,CLASS_NUM,SAMPLE_NUM_PER_CLASS,15)
                sample_dataloader = tg.get_cifar_data_loader(task,num_per_class=SAMPLE_NUM_PER_CLASS,split="train",shuffle=False)
                num_per_class = 5
                test_dataloader = tg.get_cifar_data_loader(task,num_per_class=num_per_class,split="test",shuffle=False)
                ...
                for test_images,test_labels in test_dataloader:
                    ...
                    test_features_ext = test_features.unsqueeze(0).repeat(1*CLASS_NUM,1,1,1,1)
                    test_features_ext = torch.transpose(test_features_ext,0,1)
                    test_features_ext = test_features_ext.contiguous().view(-1,FEATURE_DIM,6,6)
